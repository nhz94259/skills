...
任务文档是执行看板和验收索引，必须让读者快速判断“做什么、谁做、改哪里、怎么验、现在什么状态”：

- 总计划用表格表达 Phase、目标、依赖、验证方式和状态。
- Phase 任务用表格表达任务 ID、关联需求、Lane、写入范围、可并发、依赖、验证方式。
- 任务详情只补充表格无法表达的关键实现说明，不重复粘贴整段设计。
- 交付凭证必须有预期结果和实际结果，不能只写“已验证”。
- 测试故事面向非开发读者，步骤、预期和可观察结果必须清晰。
- 长清单超过 8 条时拆成分组或表格。
...
编写要求：
- `任务名称` 使用动词开头，避免“处理一下”“优化相关逻辑”这类模糊表达。
- `写入范围` 必须具体到文件或目录；共享文件要标为串行。
- `验证方式` 必须能被执行或人工复现，不能只写“自测”。
- 状态字段只表达当前事实，不提前标 ✅。
